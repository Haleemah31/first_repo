...
def add_data(customerID, loanID, application_data, LoanNumber, loanAmount, interestRate, TermDays, repaymentPaydate):
    try:
        with connection.cursor() as cursor:
            # Create a new record
            sql =f"INSERT INTO application_data (customerID, loanID, applicationDate, LoanNumber, loanAmount, interestRate, TermDays, repaymentDueDate, repaymentPayDate) VALUES('{customerID}', '{loanID}', '{applicationDate}', '{LoanNumber}', '{loanAmount}', '{interestRate}', '{TermDays}', '{repaymentDueDate}', '{repaymentPayDate}');"
            cursor.execute(sql)
        # connection is not autocommit by default. So you must commit to save
        # your changes.
        connection.commit
